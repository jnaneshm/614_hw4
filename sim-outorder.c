...
"                -dtlb dtlb:128:4096:32:r\n"
  ...
  opt_reg_string(odb, "-mshr",
	      ...
	      &cache_mshr_enabled, /* default */"false",
	      /* print */TRUE, /* format */NULL);

  opt_reg_int(odb, "-mshr:size",
	      "Number of MSHR registers",
	      &cache_mshr_num, /* default */-1,
	      /* print */TRUE, /* format */NULL);
		...
		//printf("ruu commit 1\n");
		  ...
		  if (cache_dl1)
			...
			cache_access(cache_dl1, Write, (LSQ[LSQ_head].addr&~3),
				     NULL, 4, sim_cycle, NULL, NULL,&LSQ[LSQ_head].mem_ready);
			if(lat < 0) {
			//mshr not available
			break;
			}
...
ruu_issue(void)
{
  int i, load_lat, tlb_lat, n_issued,mshr_unavailable;
	      ...
	      if (MD_OP_FUCLASS(rs->op) != NA)
		{
		  ...
		  if (fu && (rs->mem_ready <= sim_cycle)) //mshr or target should be free for load/store
			      ...
			      == (F_MEM|F_LOAD)))
			{
			  ...
			  if (dtlb && !mshr_unavailable && MD_VALID_ADDR(rs->addr))
			    {
